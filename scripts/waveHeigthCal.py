...
ax.set_title('Логарифмическая зависимость уровня воды от показания АЦП', loc='center', pad=10)
...
for i in range(0, 10501):
    ...
    y.append(c*(x[i]**2))
fig, ax = plt.subplots()
ax.plot(x, y, linestyle='-', linewidth = 1.5,  color= 'gold', label='height={:.4f}*adc_value^2'.format(c), zorder=1)
...
ax.set_title('Зависимость уровня воды от показания АЦП', loc='center', pad=10)
ax.set_xlim([-2, 179])
ax.set_ylim([-2, 89])
